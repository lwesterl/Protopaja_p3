/* 
 *  PRODUCTION VERSION FOR PCB V2, NO DEBUG PRINTS
 *  
...
#define dust_power_pin 15 // this is used to control dust sensor power
#define fan_power_pin 9 // this is used to control fan power
#define multi_sensor_power_pin 8 //this is used to control multisensor power
...
void manual_data_buffer(char *data, short id, int dust, struct Sensor sensor);
  ...
  if (millis() > curr_time + interval || first_connection == true){
    ...
    while(power_on){
      ...
      if (not id_problem){
        ...
        // create data buffer
        manual_data_buffer(data, id, P_tot, sensor);
...
int concentration(unsigned char *thebuf)
{
  int PM01Val;
