...
// sen eteenpäin serverille valmiiksi määritellyn wifin kautta
...
void setup(void){
 ...
 WiFi.begin("Laten-p9", "testiproto");   //WiFi connection
 ...
 void loop(void){
  ...
  if (radio.available()){
    ...
    if(WiFi.status()== WL_CONNECTED){   //Check WiFi connection status
      ...
      http.begin("http://192.168.43.98/send_string/"); //"http://jsonplaceholder.typicode.com/users");      //Specify request destination
